...
public class QNMinimizer implements Minimizer<DiffFunction>, HasEvaluators  {
  ...
  class QNInfo {
    ...
    void removeFirst() {
      // This looks expensive, but it is what the old ArrayList code would also do.
      // Instead, we probably should use a circular buffer.
  ...
  private double[] lineSearchMinPack(DiffFunction dfunc, double[] dir,
    ...
    do {
      ...
      if ((bracketed && (newPt[a] <= stpMin || newPt[a] >= stpMax))
        ...
        info = 6;
        if (!quiet)
          log.info(" line search failure: bracketed but no feasible found ");
      }
      if (newPt[a] == aMax && newPt[f] <= fTest && newPt[g] <= gTest) {
        info = 5;
        if (!quiet)
          log.info(" line search failure: sufficient decrease, but gradient is more negative ");
      }
      if (newPt[a] == aMin && (newPt[f] > fTest || newPt[g] >= gTest)) {
        info = 4;
        if (!quiet)
          log.info(" line search failure: minimum step length reached ");
      }
      if (fevals >= maxFevals) {
        // info = 3;
      ...
      if (bracketed && stpMax - stpMin <= tol * stpMax) {
        info = 2;
        if (!quiet)
          log.info(" line search failure: interval is too small ");
      }
